...
Input:
inputString: string of x length >= 0
target: substring of x length > 0

Output:
integer representing the start index of the target substring or -1 if it does not exist

What are some other insightful & revealing test cases?
- if the target length is greater than the inputString, return -1
- if the inputString length is 0, return -1

we must iterate through the inputString up to the length of the inputString - the length of target substring.
...
/*
- Here we are trying to prove the current i index is NOT the start of the target string which is why we set match = true in the outer loop
- if at any point in the inner loop we find that the char at inputString[i + j] is not equal to target[j] we know that the current i index is NOT the start of the target string and can break out of the inner loop..
*/
...
// my solution: not sure why i felt i needed a while loop over a for loop but it works
function strStr(inputString, target) {
  for (let i = 0; i < inputString.length - target.length; i++) {
    let j = 0;
    let match = true;
